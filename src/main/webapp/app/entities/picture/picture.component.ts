...
export class PictureComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.pictureService
      ...
      .subscribe(
        (res: HttpResponse<IPicture[]>) => this.paginatePictures(res.body, res.headers),
  ...
  trackId(index: number, item: IPicture) {
    return item.id;
  }
  ...
  protected paginatePictures(data: IPicture[], headers: HttpHeaders) {
